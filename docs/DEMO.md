...
- **Vendored datasets.** Three datasets are included, all baked into the image:
  - `training/data/` — AG News subset, 8k train / 2k test (~2 MB), the
    default. 4 classes: World/Sports/Business/Sci-Tech. Sized for fast CPU
    demos (~35 min).
  - `training/data-full/` — Full AG News, 120k train / 7.6k test (~30 MB).
    Same 4 classes. Pass `--dataset full`. Better for GPU runs or when you
    want higher accuracy.
  - `training/data-news/` — 20 Newsgroups, 11k train / 7k test (~22 MB).
    20 topic categories spanning computers, recreation, science, politics,
    and religion. Pass `--dataset news20`. Classic ML benchmark with clean
    Usenet post text.

  Each dataset directory can include a `labels.json` — `train.py` loads it
  automatically so the model's checkpoint knows the category names. The
  training image also bakes in the pretrained model/tokenizer at build
