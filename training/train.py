...
DEFAULT_LABEL_NAMES = {0: "World", 1: "Sports", 2: "Business", 3: "Sci/Tech"}
REPO_ROOT = pathlib.Path(__file__).resolve().parent
DEFAULT_DATA_DIR = REPO_ROOT / "data"


def load_label_names(data_dir: pathlib.Path) -> dict[int, str]:
    """Load label names from labels.json in the data directory if it exists,
    otherwise fall back to the hardcoded AG News defaults."""
...
def main() -> None:
    ...
    num_labels = args.num_labels if args.num_labels is not None else len(label_names)
    log(rank, f"Labels ({num_labels}): {label_names}")

    tokenizer = AutoTokenizer.from_pretrained(args.model_name)
    log(rank, f"Loading model '{args.model_name}' ({num_labels} labels)...")
    label_kwargs = {}
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        args.model_name, num_labels=num_labels, **label_kwargs
    ).to(device)
